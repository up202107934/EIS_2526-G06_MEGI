...
");
$check->bind_param("ii", $id_user, $id_event);
$check->execute();
$res = $check->get_result();
$row = $res->fetch_assoc();

$isInterested = $row ? intval($row["liked"]) === 1 : false;

// TOGGLE
if ($isInterested) {
    // remover interesse (unlike)
    $stmt = $db->prepare("
    ...
    // marcar interesse
    $stmt = $db->prepare("
        INSERT INTO user_events_interest (id_user, id_event, liked)
        VALUES (?, ?, 1)
    ");
    $stmt->bind_param("ii", $id_user, $id_event);
    $stmt->execute();
